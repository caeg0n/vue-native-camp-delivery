    ...
    <nb-list-item>
        <nb-left>
            ...
            <nb-body>
                ...
                <nb-text note :style="{color:'green',paddingRight:10}">R${{parseFloat(item["product"].price) | dinheiro}}
                </nb-text>
                <nb-text :style="{paddingRight:10,fontWeight: 'bold',color:'#000'}">Total R${{parseFloat(this.props.item["product"].price)*this.props.item.quantidade | dinheiro}}
                </nb-text>
            </nb-body>
...
export default {
    ...
    methods:{
        ...
        updateCart(item){
            ...
            this.updateFees()
            //this.$store.dispatch('cart/calculateFees',this.organizations)
        }
